  ...
  const computeFromInterval = () => {
    ...
    try {
      if (this.attrs.data.repeatData.type === 'months') {
        //console.log(lastRun)
        let lastRunArray = moment(lastRun).format().split('T')
        ...
        this.attrs.nextRunAt = dateForTimezone(new Date(nextRunAt))
        console.log(this.attrs.nextRunAt)
        ...
        if (nextDate.valueOf() === lastRun.valueOf()) {
          console.log('nextRun is equal to last run')
